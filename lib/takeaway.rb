...
class Takeaway
  ...
  # orders are given as a hash with two keys
  # :expected_value => 5.00, 
  ...
  def process_order(order={})
    ...
    raise "Please confirm your order." unless expected_value == value_of(dishes)
...
private
  ...
  # dishes should be an array of dish names
  # which match dishes in the menu
  def value_of(dishes=[])
    dishes.inject(0){ |sum, dish| sum + menu[dish] }
  end
